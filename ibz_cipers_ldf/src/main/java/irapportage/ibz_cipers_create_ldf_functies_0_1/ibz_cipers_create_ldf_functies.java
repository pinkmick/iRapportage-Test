...
package irapportage.ibz_cipers_create_ldf_functies_0_1;
...
/**
 * Job: ibz_cipers_create_ldf_functies Purpose: <br>
 * Description: Create all functions of iRapportage in the LDF. <br>
 * @author user@talend.com
 * @version 6.3.1.20161216_1026
 * @status 
 */
public class ibz_cipers_create_ldf_functies implements TalendJob {
...
public String getMks_port(){
	...
	private final String jobName = "ibz_cipers_create_ldf_functies";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				ibz_cipers_create_ldf_functies.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(ibz_cipers_create_ldf_functies.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class mksStruct implements routines.system.IPersistableRow<mksStruct> {
    final static byte[] commonByteArrayLock_IRAPPORTAGE_ibz_cipers_create_ldf_functies = new byte[0];
    static byte[] commonByteArray_IRAPPORTAGE_ibz_cipers_create_ldf_functies = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_IRAPPORTAGE_ibz_cipers_create_ldf_functies.length) {
				if(length < 1024 && commonByteArray_IRAPPORTAGE_ibz_cipers_create_ldf_functies.length == 0) {
   					commonByteArray_IRAPPORTAGE_ibz_cipers_create_ldf_functies = new byte[1024];
				} else {
   					commonByteArray_IRAPPORTAGE_ibz_cipers_create_ldf_functies = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_IRAPPORTAGE_ibz_cipers_create_ldf_functies, 0, length);
			strReturn = new String(commonByteArray_IRAPPORTAGE_ibz_cipers_create_ldf_functies, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_IRAPPORTAGE_ibz_cipers_create_ldf_functies) {
...
query_tPostgresqlRow_1 = "CREATE OR REPLACE FUNCTION ldf.ibz_leeftijd_in_jaren(\n    geboortedatum date,\n    datum date)\n  RETURNS integer AS\n$BODY$\nDECLARE\n  intresult int;\nBEGIN\n  intresult := 0;\n  intresult = CASE WHEN datum < geboortedatum THEN 0 ELSE extract(year \n FROM age(datum, geboortedatum)) END;\n  RETURN intresult;\nEND;\n$BODY$\n  LANGUAGE plpgsql IMMUTABLE\n  COST 100;\nALTER FUNCTION ldf.ibz_leeftijd_in_jaren(date, date)\n  OWNER TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_leeftijd_in_jaren(date, date) TO public;\nGRANT EXECUTE ON FUNCTION ldf.ibz_leeftijd_in_jaren(date, date) TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_leeftijd_in_jaren(date, date) TO ldf;";
...
query_tPostgresqlRow_2 = "CREATE OR REPLACE FUNCTION ldf.ibz_datum_naar_jaar(datum date)\n  RETURNS double precision AS\n$BODY$\nDECLARE\n  result double precision;\nBEGIN\n  result := 0;\n  result = CASE WHEN datum IS NULL THEN 0 ELSE floor(extract(year FROM datum)) END;\n  RETURN result;\nEND;\n$BODY$\n  LANGUAGE plpgsql IMMUTABLE\n  COST 100;\nALTER FUNCTION ldf.ibz_datum_naar_jaar(date)\n  OWNER TO  mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaar(date) TO public;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaar(date) TO postgres;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaar(date) TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaar(date) TO ldf;";
...
query_tPostgresqlRow_3 = "CREATE OR REPLACE FUNCTION ldf.ibz_datum_naar_jaarmaand(datum date)\n  RETURNS double precision AS\n$BODY$\nDECLARE\n  result double precision;\nBEGIN\n  result := 0;\n  result = CASE WHEN datum IS NULL THEN 0 ELSE floor(extract(year \n FROM datum)*100 + extract(month FROM datum)) END;\n  RETURN result;\nEND;\n$BODY$\n  LANGUAGE plpgsql IMMUTABLE\n  COST 100;\nALTER FUNCTION ldf.ibz_datum_naar_jaarmaand(date)\n  OWNER TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaarmaand(date) TO public;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaarmaand(date) TO postgres;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaarmaand(date) TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaarmaand(date) TO ldf;";
...
query_tPostgresqlRow_4 = "CREATE OR REPLACE FUNCTION ldf.ibz_datum_naar_jaarmaanddag(datum date)\n  RETURNS double precision AS\n$BODY$\nDECLARE\n  result double precision;\nBEGIN\n  result := 0;\n  result = CASE WHEN datum IS NULL THEN 0 ELSE floor(extract(year \n FROM datum)*10000 + extract(month FROM datum)*100 + extract(day FROM datum)) END;\n  RETURN result;\nEND;\n$BODY$\n  LANGUAGE plpgsql IMMUTABLE\n  COST 100;\nALTER FUNCTION ldf.ibz_datum_naar_jaarmaanddag(date)\n  OWNER TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaarmaanddag(date) TO public;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaarmaanddag(date) TO postgres;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaarmaanddag(date) TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_naar_jaarmaanddag(date) TO ldf;";
...
query_tPostgresqlRow_5 = "CREATE OR REPLACE FUNCTION ldf.ibz_datum_tussen_strings_incl(\n    datum_start character,\n    datum date,\n    datum_eind character)\n  RETURNS boolean AS\n$BODY$\nDECLARE\n  booleanresult boolean;\nBEGIN\n  booleanresult := false;\n  booleanresult = CASE WHEN (datum <= to_date(datum_eind,\n 'DD-MM-YYYY') AND datum >= to_date(datum_start,'DD-MM-YYYY')) THEN true ELSE false END;\n  RETURN booleanresult;\nEND;\n$BODY$\n  LANGUAGE plpgsql IMMUTABLE\n  COST 100;\nALTER FUNCTION ldf.ibz_datum_tussen_strings_incl(character, date, character)\n  OWNER TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_tussen_strings_incl(character, date, character) TO public;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_tussen_strings_incl(character, date,\n  character) TO postgres;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_tussen_strings_incl(character, date,\n  character) TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_datum_tussen_strings_incl(character, date, character) TO ldf;";
...
query_tPostgresqlRow_6 = "CREATE OR REPLACE FUNCTION ldf.ibz_opgemaaktedatum(\n    datum date,\n    indicator text,\n    formaat boolean)\n  RETURNS text AS\n$BODY$\nDECLARE strresult text;\nBEGIN\n  strresult := '';\n  IF indicator = 'V' THEN\n    IF $3 = true THEN\n      strresult := to_char(datum,'DD-MM-YYYY');\n    ELSE \n      strresult := to_char(datum,'YYYY-MM-DD');\n    END IF;\n  ELSEIF indicator = 'D' THEN\n    IF $3 = true THEN\n      strresult := to_char(datum,'00-MM-YYYY');\n    ELSE \n      strresult := to_char(datum,'YYYY-MM-00');\n    END IF;\n  ELSEIF indicator = 'M' THEN\n    IF $3 = true THEN\n      strresult := to_char(datum,'00-00-YYYY');\n    ELSE \n      strresult := to_char(datum,'YYYY-00-00');\n    END IF;\n  ELSEIF indicator = 'J' THEN\n    IF $3 = true THEN\n      strresult := '00-00-0000';\n    ELSE \n      strresult := '0000-00-00';\n    END IF;\n  ELSE\n    IF $3 = true THEN\n      strresult := '-';\n    ELSE \n      strresult := '-';\n    END IF;\n  END IF;\n  RETURN strresult;\nEND;\n$BODY$\n  LANGUAGE plpgsql IMMUTABLE\n  COST 100;\nALTER FUNCTION ldf.ibz_opgemaaktedatum(date, text, boolean)\n  OWNER TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_opgemaaktedatum(date, text, boolean) TO public;\nGRANT EXECUTE ON FUNCTION ldf.ibz_opgemaaktedatum(date, text, boolean) TO postgres;\nGRANT EXECUTE ON FUNCTION ldf.ibz_opgemaaktedatum(date, text, boolean) TO mks_owner;\nGRANT EXECUTE ON FUNCTION ldf.ibz_opgemaaktedatum(date, text, boolean) TO ldf;";
 ...
 */
    ...
    public static void main(String[] args){
        final ibz_cipers_create_ldf_functies ibz_cipers_create_ldf_functiesClass = new ibz_cipers_create_ldf_functies();

        int exitCode = ibz_cipers_create_ldf_functiesClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = ibz_cipers_create_ldf_functies.class.getClassLoader().getResourceAsStream("irapportage/ibz_cipers_create_ldf_functies_0_1/contexts/"+contextStr+".properties");
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : ibz_cipers_create_ldf_functies");
        }
...
/************************************************************************************************
 *     72570 characters generated by Talend Open Studio for Data Integration 
